...
function DataVisualization() {
  ...
  useEffect(() => {
    ...
    if (selectedMonth) {
      axios
        .get(`http://161.35.190.125:3002/get-budgets/${userId}?month=${selectedMonth}`, {
          headers: {
  ...
  useEffect(()=>{
    ...
    if (selectedMonth) {
      axios
        .get(`http://161.35.190.125:3002/get-expenses/${userId}?month=${selectedMonth}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  return (
    <main className="center" id="main" aria-label="main">
      <div>
        ...
        <select id="months" onChange={(e) => setSelectedMonth(e.target.value)} value={selectedMonth} style={{ marginRight: '15px', fontSize: '18px' }}>
          <option value="" style={{ fontSize: '18px' }}>Select Month</option>
          {months.map((month) => (
